...
# Client for the OCMFET acquisition system developed by Elbatech (zero version)
...
def bytes2samples(data):
    ...
    # print(f"p1: {p1[0]:06x} -> {points[0]:06x} -> {I[0]}")
    # print(f"p2: {p2[0]:06x} -> {points[1]:06x} -> {I[1]}")
...
class UDPClientGUI(QMainWindow):
    ...
    def initUI(self):
        ...
        for i in range(self.n_channels):
            ...
            self.Vs_controls[i]["spin_box"] = QDoubleSpinBox(self)
            self.Vs_controls[i]["spin_box"].setRange(-4, 0)
            self.Vs_controls[i]["spin_box"].setSingleStep(0.1)
            self.Vs_controls[i]["spin_box"].setDecimals(2)
            self.Vs_controls[i]["spin_box"].setValue(0.1)
            self.Vs_controls[i]["spin_box"].setSuffix(" V")
            self.Vs_controls[i]["set_button"] = QPushButton("Set", self)
            self.Vs_controls[i]["reset_button"] = QPushButton("Reset", self)
            ...
            self.Vs_controls[i]["set_button"].clicked.connect(
                lambda _, channel=i: self.set_Vs(
                    channel, self.Vs_controls[channel]["spin_box"].value()))
            self.Vs_controls[i]["reset_button"].clicked.connect(
        ...
        for i in range(self.n_channels):
            ...
            layout.addWidget(QLabel("V" + sub("G")), 0, 0)
            layout.addWidget(self.Vg_controls[i]["spin_box"], 1, 0)
            layout.addWidget(self.Vg_controls[i]["set_button"], 2, 0)
            layout.addWidget(self.Vg_controls[i]["reset_button"], 3, 0)

            layout.addWidget(QLabel("V" + sub("S")), 0, 1)
            layout.addWidget(self.Vs_controls[i]["spin_box"], 1, 1)
            layout.addWidget(self.Vs_controls[i]["set_button"], 2, 1)
            layout.addWidget(self.Vs_controls[i]["reset_button"], 3, 1)
...
# ______________________________________________________________
        self.T2_line_edit = QLineEdit(self)
        self.T2_set_button = QPushButton("Set", self)
        self.T2_set_button.clicked.connect(
            lambda: self.send_command(f"sT2 {float(self.T2_line_edit.text())}"))
        self.time_layout.addWidget(self.T2_line_edit)
        self.time_layout.addWidget(self.T2_set_button)
# ______________________________________________________________
    ...
    def reset_Vs(self, channel):
        self.Vs_controls[channel]["spin_box"].setValue(0)
...
class DataListener(QThread):
    ...
    def update_bytes_to_emit(self, fs, tr):
        self.bytes_to_emit = int(144*fs*tr*1e-7)*4
